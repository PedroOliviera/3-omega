...
def n_scans(lc_list=[100],scan_v_or_f='freq',harm=3,stb_time=9,averaged_time=1,n_sweeps=1,lc_const=3,direc=''):
    ...
    for ctr in range(n_sweeps):
        ...
        if direc != '':
            dat = [lc_list,a]
            tit = ['freq','v3w']
            csv_writer('Sweep' + str(ctr+1),dat,tit,direc=direc)
    
    return scan_val_x, scan_val_y

def mn_scans(lc_list,scan_v_or_f='freq',harm=3,stb_time=9,averaged_time=1,n_sweeps=1,lc_const=[1,2,3],direc=''):
   ...
   if type(lc_const) != list:
       lc_const = [lc_const]
   for ctr in lc_const:
       a,b = n_scans(lc_list,scan_v_or_f,harm,stb_time,averaged_time,n_sweeps,ctr,direc)
       scan_val_x.append(a)
       scan_val_y.append(b)
...
def fullScan(start,end,num_points,scan_v_or_f,stb_time,averaged_time,n_sweeps,lc_const,direc):
    '''
    ...
    v3w_x, v3w_y = mn_scans(lc_list,scan_v_or_f = scan_v_or_f,harm = 3, stb_time = stb_time, averaged_time = averaged_time
                            , n_sweeps = n_sweeps,lc_const = lc_const,direc=direc) 
