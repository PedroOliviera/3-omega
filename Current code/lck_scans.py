...
#scan_v_or_f
    ...
    for i in range(1,len(lc_list)+1):
        ...
        ti = time.time()
        print('For:   ' + str(lc_list[i-1]) + unit)
        a,b = getData()
...
def n_scans(lc_list=[100],scan_v_or_f='freq',harm=3,stb_time=9,averaged_time=1,n_sweeps=1,lc_const=3):
    ...
    for ctr in range(n_sweeps):
        print('Starting sweep number: ' + str(ctr+1))
...
def mn_scans(lc_list,scan_v_or_f='freq',harm=3,stb_time=9,averaged_time=1,n_sweeps=1,lc_const=[1,2,3]):
   ...
   returns list within a list within a list
   '''
...
def fullScan(start,end,num_points,scan_v_or_f,stb_time,averaged_time,n_sweeps,lc_const):
    ...
    if scan_v_or_f == 'freq':
        ...
        lc_list = getListVolt(start,end,num_points)
    ...
    beep()
    #first do v3w scan
    input('make sure BNCs are set to sample')
    tell_lockin('SENS22')
    ...
    return output, lc_list, v3w_x, v3w_avg, v1w_x,v1w_sh_x ,lc_const
...
direc = create_folder('A1_Scan1-2019-08-19',direc)

#EXAMPLE OF SAVE
#csv_writer('NAME OF FILE',data,title,direc=direc)

#output = fullScan(200,'whatever',50,scan_v_or_f='freq',stb_time=9,averaged_time=3,n_sweeps=50,lc_const=[3])


#output = fullScan(200,18000,40,scan_v_or_f='freq',stb_time=9,averaged_time=3,n_sweeps=4,lc_const=[1.5,1.75,2,2.25,2.5,2.75,3,3.25])
output = fullScan(500,3700,9,scan_v_or_f='freq',stb_time=9,averaged_time=3,n_sweeps=4,lc_const=[3])
...
csv_writer('A1-SiOx-20',data4ara,title4ara,direc=direc)
csv_writer('A1-SiOx-20_analysis',data4ped,title4ped,direc=direc)

'''
output = fullScan(4000,14000,10,scan_v_or_f='freq',stb_time=8,averaged_time=3,n_sweeps=5,lc_const=1.5)

title4ara = ['f_all','Vs_3w','Vs_1w','Vs_1w_o','Vsh_1w']
title4ped = ['v3w','v3w_avg','v1w','v1w_sh_x']

data4ara = output[0]
data4ped = [output[i] for i in range(1,len(output))]

csv_writer('TARA3_BCB-5',data4ara,title4ara,direc=direc)
csv_writer('TARA3_BCB-5_analysis',data4ped,title4ped,direc=direc)
'''
