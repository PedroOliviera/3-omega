...
k=21
for i in range(10):
    y.append(calibrate(stb2,10,listvolts[j],listvolts[k],'SLVL'))
    x.append(i+1)
    
fig = plt.figure()
plt.errorbar(x,y,fmt='o') 
fig.suptitle('Time to stabilize at different frequencies (1\u03C9)', fontsize=20)
plt.xlabel('Frequency', fontsize=16)
plt.ylabel('Time to stabilize', fontsize=16)
plt.legend(['forward'])
fig.savefig('calibrate.jpg')
plt.show()
lock.close()
